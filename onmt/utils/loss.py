...
class LossComputeBase(nn.Module):
    ...
    def _compute_loss(self, batch, text_out, image_out, **kwargs):
        """
    ...
    def __call__(self,
        ...
        if shard_size == 0:
            loss, stats = self._compute_loss(batch, text_out, image_out, **shard_state)
            return loss / float(normalization), stats
        batch_stats = onmt.utils.Statistics()
        for shard in shards(shard_state, shard_size):
            loss, stats = self._compute_loss(batch, text_out, image_out, **shard)
...
class NMTLossCompute(LossComputeBase):
    ...
    def __init__(self, criterion, generator, normalization="sents",
        ...
        self.cosine_loss = nn.CosineEmbeddingLoss(margin=0.3)
        self.lamb_0 = 0.3
    ...
    def _compute_loss(self, batch, text_out, image_out, output, target, std_attn=None,
        ...
        # mse_loss = self.mse_loss(image_out, text_out)
        # hinge_loss = self.hinge_loss(image_out, text_out)
        cosine_loss = self.cosine_loss(image_out, text_out, torch.Tensor([-1]).cuda())
        loss = self.criterion(scores, gtruth)
        loss = loss + cosine_loss*100
        # loss = loss + mse_loss*self.lamb_0
