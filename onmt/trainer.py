...
def build_trainer(opt, device_id, model, fields, optim, model_saver=None):
    ...
    train_file = tables.open_file('data/flickr30k_train_vgg19_bn_cnn_features.hdf5', mode='r')
    valid_file = tables.open_file('data/flickr30k_valid_vgg19_bn_cnn_features.hdf5', mode='r')
    # global features
    train_img_feats = train_file.root.global_feats[:]
    valid_img_feats = valid_file.root.global_feats[:]
    # local spatial features
    train_img_feats_local = train_file.root.local_feats[:]
    valid_img_feats_local = valid_file.root.local_feats[:]
    ...
    trainer = onmt.Trainer(model, train_loss, valid_loss, optim, trunc_size,
                           ...
                           train_img_feats_local=train_img_feats_local,
                           valid_img_feats_local=valid_img_feats_local,
...
class Trainer(object):
    ...
    def __init__(self, model, train_loss, valid_loss, optim,
                 ...
                 train_img_feats_local=None, valid_img_feats_local=None,
        ...
        self.train_img_feats_local = train_img_feats_local
        self.valid_img_feats_local = valid_img_feats_local
    ...
    def validate(self, valid_iter, moving_average=None):
        ...
        with torch.no_grad():
            ...
            for batch in valid_iter:
                ...
                img_feats_local = torch.from_numpy(self.valid_img_feats_local[idxs])
                img_feats_local= torch.Tensor(img_feats_local)
                img_feats_local = img_feats_local.view(-1, 512, 49)
  
                if next(valid_model.parameters()).is_cuda:
                    img_feats = img_feats.cuda()
                    img_feats_local = img_feats_local.cuda()
                else:
                    img_feats = img_feats.cpu()
                    img_feats_local = img_feats_local.cpu()
                ...
                with torch.cuda.amp.autocast(enabled=self.optim.amp):
                    ...
                    outputs, attns, text_out, image_out = valid_model(src, tgt, src_lengths, 
                                                img_feats, img_feats_local,
                                                with_align=self.with_align)

                    # Compute loss.
                    _, batch_stats = self.valid_loss(batch, outputs, attns, text_out, image_out)
        ...
        for name, param in valid_model.named_parameters():
            if 'lamb_0' in name:
                print(name, param)
                break
    ...
    def _gradient_accumulation(self, true_batches, normalization, total_stats,
        ...
        for k, batch in enumerate(true_batches):
            ...
            img_feats_local = torch.from_numpy(self.train_img_feats_local[idxs])
            img_feats_local= torch.Tensor(img_feats_local)
            img_feats_local = img_feats_local.view(-1, 512, 49)
            ...
            for j in range(0, target_size-1, trunc_size):
                ...
                if self.accum_count == 1:
                    self.optim.zero_grad()
                with torch.cuda.amp.autocast(enabled=self.optim.amp):
                    outputs, attns, text_out, image_out = self.model(
                        src, tgt, src_lengths, img_feats, 
                        img_feats_local, bptt=bptt,
                        with_align=self.with_align)
                    bptt = True
